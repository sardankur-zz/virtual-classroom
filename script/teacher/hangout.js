...
var hangout = function (config1) {
    ...
    function openDefaultSocket() {
        defaultSocket = config1.openSocket({onmessage: onDefaultSocketResponse});
    }

    function onDefaultSocketResponse(response) {
        if (response.userToken == self.userToken) return;

        if (isGetNewRoom && response.roomToken && response.broadcaster) config1.onRoomFound(response);
    ...
    function openSubSocket(_config) {
        ...
        var socket = config1.openSocket(socketConfig),
        ...
        var peerConfig = {
            ...
            onChannelMessage: function (event) {
                config1.onChannelMessage(JSON.parse(event.data));
            }
        };
        ...
        function onChannelOpened(channel) {
            ...
            channel.send(JSON.stringify({
                message: '<sub>' + self.userName + ' has joined</sub>',
                sender: self.userName
            }));

            if (config1.onChannelOpened) config1.onChannelOpened(channel);
